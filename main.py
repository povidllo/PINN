...
from fastapi import (
    FastAPI, APIRouter, WebSocket, WebSocketDisconnect, Request, Body
)
...
async def websocket_endpoint(websocket: WebSocket):
    try:
        await neural_net_manager.ws_manager.connect(websocket, ms.glob_user)
        while True:
            await asyncio.sleep(0.002)
            data = await websocket.receive_text()
    except WebSocketDisconnect:
        print("WebSocketDisconnect", type(websocket))
        neural_net_manager.ws_manager.disconnect(websocket, ms.glob_user)
    except Exception as err:
        print(f"WebSocket error: {err}")


async def get_host():
    return {"cur_host": ms.cur_host}


@router.post("/create_model")
async def create_neural_model_post(params: ms.mHyperParams = Body(...)):
    params.save_weights_path = params.save_weights_path or "weights/osc_1d.pth"
    await neural_net_manager.create_model(params)
    neural_list = await ms.mNeuralNetMongo.get_all()

    table_html = templates.get_template("html/table_template.html").render({"items": neural_list})
    letter = ms.ChatMessage(user=ms.glob_user, msg_type='jinja_tmpl', data=['model_desc', table_html])
...
async def load_model_handler(model_id: ms.mNeuralNetMongo = Body(...)):
...
async def run_neural_net_pict():
    ...
    letter = ms.ChatMessage(user=ms.glob_user, msg_type='jinja_tmpl', data=['chat_container_id', image_html])
    await neural_net_manager.ws_manager.send_personal_message_json(letter)
...
async def clear_database():
    try:
        await ms.clear_all_collections()
...
async def update_optimizer_params(optimizer_name: dict = Body(...)):
    ...
    letter = ms.ChatMessage(user=ms.glob_user, msg_type='jinja_tmpl', data=['optimizer-params', optimizer_params_html])
    await neural_net_manager.ws_manager.send_personal_message_json(letter)
...
def main(loop):
    ...
    async def lifespan(app: FastAPI):
        await init_beanie(database=client[ms.database_name], document_models=[
            ms.MongoEstimate, ms.MongoRecord, ms.mOptimizerMongo, ms.mDataSetMongo,
            ms.mHyperParamsMongo, ms.mNeuralNetMongo, ms.mWeightMongo
        ])
        yield
    ...
    import uvicorn
    config = uvicorn.Config(app, host=ms.cur_host, port=8010, loop=loop, access_log=False)
    server = uvicorn.Server(config)
    loop.run_until_complete(server.serve())


if __name__ == '__main__':
    client = AsyncIOMotorClient(f"mongodb://{ms.database_ulr}:27017")
